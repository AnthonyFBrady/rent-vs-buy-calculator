...
export function StepShelters({ inputs, patch }: Props) {
      ...
      >
        ...
        <div style={{ paddingTop: '16px', paddingBottom: '16px', borderBottom: '1px solid var(--color-outline)' }}>
          <Toggle
            ...
            label="TFSA"
            description={`$7k/yr new room. Tax-free growth, no capital gains at exit. Estimated lifetime room: ${fmtCAD.format(Math.min(95_000, Math.max(0, (2026 - Math.max((inputs.birthYear ?? 1990) + 18, 2009)) * 7_000)))}.`}
          />
        </div>
        ...
        <div style={{ paddingTop: '16px', paddingBottom: '16px', borderBottom: '1px solid var(--color-outline)' }}>
          <Toggle
            ...
            label="FHSA"
            description="$8k/yr tax-deductible, tax-free growth. Up to $40k lifetime. First-time buyers only."
          />
          {useFHSA && (
          ...
          {(inputs.monthlyRentalIncome ?? 0) > 0 && (
            <div style={{ marginTop: '12px' }}>
              <RangeInput
                ...
                minLabel="$200"
                maxLabel="$4k"
                description="Enter income already net of expected vacancy and expenses."
              />
            </div>
          )}
        </div>
